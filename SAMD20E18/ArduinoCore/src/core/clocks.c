...
/* Note: Caller is responsible for knowing the limitations of dividers and setup for each of
 * the various clock sources, generators, and dividers */
                          ...
                          uint8_t runInStdBy, uint8_t outPutToPin )
{
  ...
  if( clkSrc > GCLK_GENCTRL_SRC_DFLL48M_Val )
    return -1;
  ...
  if( runInStdBy )
    genCtrlReg |= GCLK_GENCTRL_RUNSTDBY;
  if( div != 0 )
    genCtrlReg |= GCLK_GENCTRL_DIVSEL;
  if( outPutToPin )
    genCtrlReg |= GCLK_GENCTRL_OE;
...
{
  if( genClk > GCLK_CLKCTRL_GEN_GCLK7_Val )
    return -1;
  if( id > GCLK_CLKCTRL_ID_PTC_Val )
    return -1;

  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID( id ) | GCLK_CLKCTRL_GEN( genClk ) | GCLK_CLKCTRL_CLKEN;
  GCLK_WAIT_SYNC;
