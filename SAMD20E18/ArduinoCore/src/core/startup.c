 ...
 * 0 to DFLL48M. CPU will run at 48MHz. 6) Modify PRESCaler value of OSCM to
 * have 8MHz 7) Put OSC8M as source for Generic Clock Generator 3
 */
// Constants for Clock generators
#define GENERIC_CLOCK_GENERATOR_MAIN ( 0u )
#define GENERIC_CLOCK_GENERATOR_XOSC32K ( 1u )
#define GENERIC_CLOCK_GENERATOR_OSC32K ( 1u )
#define GENERIC_CLOCK_GENERATOR_OSCULP32K \
    ( 2u ) /* Initialized at reset for WDT */
...
{
    /* Set 1 Flash Wait State for 48MHz, cf tables 20.9 and 35.27 in SAMD21
     * Datasheet */
...
#define FUSES_OSC32K_CAL_ADDR \
    ( uint32_t )( 0x806020 + 38 ) // SAMD20 datasheet section 9.5, table 9-4
    ...
    SYSCTRL->OSC32K.reg =
        ...
        SYSCTRL_OSC32K_STARTUP(
            0x6u ) | // cf table 15.10 of product datasheet in chapter 15.8.6
     ...
     */
    SYSCTRL->XOSC32K.reg =
        SYSCTRL_XOSC32K_STARTUP(
            0x6u ) | /* cf table 15.10 of product datasheet in chapter 15.8.6 */
        SYSCTRL_XOSC32K_XTALEN |
        SYSCTRL_XOSC32K_EN32K;
    SYSCTRL->XOSC32K.bit.ENABLE =
        1; /* separate call, as described in chapter 15.6.3 */
    ...
    /* Note: Due to synchronization, there is a delay from writing CTRL.SWRST
     * until the reset is complete. CTRL.SWRST and STATUS.SYNCBUSY will both be
     * cleared when the reset is complete, as described in chapter 13.8.1
     */
    ...
    GCLK->GENDIV.reg = GCLK_GENDIV_ID(
        GENERIC_CLOCK_GENERATOR_XOSC32K ); // Generic Clock Generator 1
    ...
    GCLK->GENCTRL.reg =
        GCLK_GENCTRL_ID(
            GENERIC_CLOCK_GENERATOR_OSC32K ) | // Generic Clock Generator 1
#if defined( CRYSTALLESS )
        GCLK_GENCTRL_SRC_OSC32K | // Selected source is Internal 32KHz
                                  // Oscillator
#else
        GCLK_GENCTRL_SRC_XOSC32K | // Selected source is External 32KHz
                                   // Oscillator
#endif
        //                      GCLK_GENCTRL_OE | // Output clock to a pin for
        //                      tests
        GCLK_GENCTRL_GENEN;
    ...
    GCLK->CLKCTRL.reg =
        GCLK_CLKCTRL_ID(
            GENERIC_CLOCK_MULTIPLEXER_DFLL48M ) | // Generic Clock Multiplexer 0
     ...
     * chapter 15.6.7.1 - Closed-Loop Operation */

    /* Remove the OnDemand mode, Bug
    ...
    SYSCTRL->DFLLMUL.reg =
        SYSCTRL_DFLLMUL_CSTEP(
            31 ) | // Coarse step is 31, half of the max value
        SYSCTRL_DFLLMUL_FSTEP(
            511 ) | // Fine step is 511, half of the max value
        SYSCTRL_DFLLMUL_MUL(
            ( VARIANT_MCK + VARIANT_MAINOSC / 2 ) /
            VARIANT_MAINOSC ); // External 32KHz is the reference
...
#define NVM_SW_CALIB_DFLL48M_COARSE_VAL 58
    ...
    // TODO(tannewt): Load this value from memory we've written previously.
    // There isn't a value from the Atmel factory.
    uint32_t fine = 0x1ff;
    ...
    SYSCTRL->DFLLMUL.reg =
        SYSCTRL_DFLLMUL_CSTEP( 0x1f /
                               4 ) | // Coarse step is 31, half of the max value
     ...
     */
    GCLK->GENDIV.reg = GCLK_GENDIV_ID(
        GENERIC_CLOCK_GENERATOR_MAIN ); // Generic Clock Generator 0
     ...
     */
    GCLK->GENDIV.reg = GCLK_GENDIV_ID(
        GENERIC_CLOCK_GENERATOR_OSC8M ); // Generic Clock Generator 3

    /* Write Generic Clock Generator 3 configuration */
    GCLK->GENCTRL.reg =
        GCLK_GENCTRL_ID(
            GENERIC_CLOCK_GENERATOR_OSC8M ) | // Generic Clock Generator 3
        GCLK_GENCTRL_SRC_OSC8M | // Selected source is RC OSC 8MHz (already
                                 // enabled at reset)
        //                      GCLK_GENCTRL_OE | // Output clock to a pin for
        //                      tests
        GCLK_GENCTRL_GENEN;
    ...
    /*
     * Now that all system clocks are configured, we can set CPU and APBx BUS
     * clocks. There values are normally the one present after Reset.
     */
...
/*
 * The low power clock system is all based off the external 32.768 kHz crystal
 * (or the internal). The external crystal drives clock generator 1 and 2. Clock
 * generator 1 is the source for the DFLL 48 MHz controller for the main CPU
 * which drives clock generator 0.
 */
 ...
 */
    ...
    ///*
    ///----------------------------------------------------------------------------------------------
    //* 4) Put Generic Clock Generator 1 as source for Generic Clock Multiplexer
    //0 (DFLL48M reference)
    //*/
