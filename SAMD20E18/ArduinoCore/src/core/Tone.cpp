...
void tone( uint32_t outputPin, uint32_t frequency, uint32_t duration )
{
    ...
    //
    // Calculate best prescaler divider and comparator value for a 16 bit TC
    // peripheral
    //
    ...
    portToggleRegister =
        &( PORT->Group[g_APinDescription[outputPin].ulPort].OUTTGL.reg );
    portClearRegister =
        &( PORT->Group[g_APinDescription[outputPin].ulPort].OUTCLR.reg );
