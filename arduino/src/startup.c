 ...
 * application. Each module (RTC, WDT, GPIO etc..) automatically enables the
 * module when called. The low power system initializes the 32 kHz crystal
 * oscillator to drive Generic Clock 1, which provides the RTC with a clock
 * source. Generic Clock 0 ( GCLK Main ) is driven by the OSCM8 at 8 MHz. The
 * user can change the main clock frequency by calling changeCPUClk() in the
 * application, which will allow them to run the DFLL at 48 MHz or the OSCM8 at
 * a lower speed.
 */
